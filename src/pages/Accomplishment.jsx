  ...
  return <>
    ...
    <form className="modern-accomplishment-paper" onSubmit={save}>
      <div className="form-cover">
        ...
        <div className="cover-title"><span>BRANCH VISIT</span><strong>ACCOMPLISHMENT FORM</strong><p>Branch visit documentation • A4 printable record</p></div>
    ...
    <div className="recent-accomplishments no-print"><div className="page-title-row"><div><span className="eyebrow">HISTORY</span><h2>Recent Accomplishments</h2></div></div><div className="content-card table-wrap"><table><thead><tr><th>DATE</th><th>BRANCH</th><th>TEAM LEADER</th><th>HMS STAFF</th><th>CREATED</th></tr></thead><tbody>{recent.length?pageItems.map(r=><tr key={r.id}><td>{r.date||'—'}</td><td><b>{r.branchName||r.branchVisited||'—'}</b></td><td>{r.teamLeader||'—'}</td><td>{r.hmsStaff||'—'}</td><td>{r.createdAt?.toDate?r.createdAt.toDate().toLocaleString():'Saved'}</td></tr>):<tr><td colSpan="5" className="empty-state">No saved accomplishment forms yet.</td></tr>}</tbody></table>{recent.length>0&&<TablePagination {...pagination} totalItems={recent.length}/>}</div></div>
  </>;
}

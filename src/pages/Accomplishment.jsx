...
export default function Accomplishment({ embedded=false, onSaved }={}){
  const { profile } = useAuth();
  const [branches,setBranches]=useState([]), [groups,setGroups]=useState([]), [leaders,setLeaders]=useState([]), [form,setForm]=useState(blank), [saving,setSaving]=useState(false), [loading,setLoading]=useState(true), [error,setError]=useState(''), [saved,setSaved]=useState(false), [recent,setRecent]=useState([]), [controlNumber,setControlNumber]=useState('');
  useEffect(()=>{if(!profile)return;(async()=>{try{const bq=query(collection(db,'branches'),orderBy('createdAt','desc'));const snap=await getDocs(bq);const allBranches=snap.docs.map(d=>({id:d.id,...d.data()}));const userGroupId=profile.groupId||'';const userGroupName=String(profile.groupName||'').trim().toLowerCase();const visibleBranches=profile.role==='super_admin'?allBranches:allBranches.filter(b=>{const branchGroupId=String(b.groupId||'');const branchGroupName=String(b.groupName||'').trim().toLowerCase();return (userGroupId&&branchGroupId===String(userGroupId))||(userGroupName&&branchGroupName===userGroupName)});setBranches(visibleBranches);const g=await getDocs(query(collection(db,'groups'),orderBy('name','asc')));setGroups(g.docs.map(d=>({id:d.id,...d.data()})));const u=await getDocs(query(collection(db,'users'),where('position','==','Team Leader')));setLeaders(u.docs.map(d=>({id:d.id,...d.data()})).filter(x=>String(x.position||x.role||'').toLowerCase().replace(/[_-]/g,' ').includes('team leader')&&x.active!==false));const aq=profile.role==='super_admin'?query(collection(db,'accomplishments'),orderBy('createdAt','desc')):query(collection(db,'accomplishments'),where('groupId','==',profile.groupId||'unassigned'));const a=await getDocs(aq);const recentItems=a.docs.map(d=>({id:d.id,...d.data()})).sort((x,y)=>(y.createdAt?.seconds||0)-(x.createdAt?.seconds||0));setRecent(recentItems.slice(0,8));}catch(e){setError(e.message)}finally{setLoading(false)}})()},[profile]);
        ...
        <div className="lav-brand-row">
          ...
          <div className="lav-header-fields no-print">
            ...
            <label>BRANCH<select value={form.branchId} onChange={e=>{const b=branches.find(x=>x.id===e.target.value);change('branchId',e.target.value);change('branchVisited',b?.branchName||'')}} required>
              <option value="">Select your group branch...</option>

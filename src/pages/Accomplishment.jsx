...
export default function Accomplishment(){
  ...
  const [branches,setBranches]=useState([]), [leaders,setLeaders]=useState([]), [form,setForm]=useState(blank), [saving,setSaving]=useState(false), [loading,setLoading]=useState(true), [error,setError]=useState(''), [saved,setSaved]=useState(false), [recent,setRecent]=useState([]), [printPreview,setPrintPreview]=useState(false);
    ...
    {printPreview&&<div className="print-preview-overlay no-print" role="dialog" aria-modal="true">
      <div className="print-preview-dialog">
        <div className="print-preview-head">
          <div><b>Print Preview</b><small>Letter Size • 8.5 × 11 in • Portrait</small></div>
          <button type="button" className="modal-close" onClick={closePrintPreview} aria-label="Close">×</button>
        </div>
          ...
          <div className="print-preview-sheet">
            <div className="print-preview-content">
              ...
              <section className="form-section visit-section">
                <div className="section-heading"><span>01</span><div><b>VISIT INFORMATION</b><small>Basic details of the branch visit and assigned personnel</small></div></div>
                <div className="form-grid four">
                  <div className="field span-2"><span>BRANCH VISITED</span><div className="preview-field">{branch?.branchName || 'Select branch...'}</div></div>
                  <div className="field"><span>DATE</span><div className="preview-field">{fmtDate(form.date)}</div></div>
                  <div className="field"><span>TEAM LEADER</span><div className="preview-field">{val(form.teamLeader)}</div></div>
        ...
        <div className="print-preview-actions">
          <button type="button" className="outline-btn" onClick={closePrintPreview}>Close Preview</button>
          <button type="button" className="amber-btn" onClick={()=>window.print()}>🖨 Print</button>
        </div>
      </div>
    </div>}

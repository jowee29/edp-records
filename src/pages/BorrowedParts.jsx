...
export default function UsedParts(){
 ...
 const markReturned=x=>{if(x.status==='RETURNED')return;setConfirm({title:'Mark as Returned',message:`Markahan na RETURNED ang borrowed parts ni ${x.borrower||'borrower'}?\n\nIbabalik ang quantity sa Parts Inventory.`,confirmLabel:'Mark Returned',danger:false,onConfirm:async()=>{setConfirmSaving(true);try{
  await runTransaction(db,async tx=>{const ref=doc(db,'borrowedParts',x.id);for(const p of x.borrowedParts||[]){if(!p.inventoryId)continue;const ir=doc(db,'partsInventory',p.inventoryId);const s=await tx.get(ir);if(s.exists())tx.update(ir,{quantity:(Number(s.data()?.quantity)||0)+Number(p.quantity||0),updatedAt:serverTimestamp()});}tx.update(ref,{status:'RETURNED',updatedAt:serverTimestamp()})});await audit({action:'RETURN_BORROWED_PART',details:`Marked borrowed parts as returned for ${x.borrower||''}`,targetUserId:x.id});closeView();await load();
 }catch(e){setError(e.message||'Unable to mark returned.')}finally{setConfirmSaving(false);setConfirm(null)}}});};

...
export default function PartsInventory(){
  ...
  const [editing,setEditing]=useState(null),[modalOpen,setModalOpen]=useState(false),[viewing,setViewing]=useState(null);
  ...
  return <>
    ...
    <div className="content-card table-wrap parts-table"><table><thead><tr><th>ITEM CODE</th><th>DESCRIPTION</th><th>CONTROL / SERIAL NO.</th><th>BRANCH</th><th>QUANTITY</th><th>PRICE</th><th>TOTAL VALUE</th></tr></thead><tbody>{loading?<tr><td colSpan="7" className="branch-empty">Loading Parts Inventory...</td></tr>:shown.length===0?<tr><td colSpan="7" className="branch-empty"><div className="branch-empty-icon">◌</div><strong>No inventory records found</strong><p>Add your first parts inventory item.</p><button className="amber-btn" onClick={openAdd}>＋ Add Inventory Item</button></td></tr>:shown.map(x=><tr key={x.id} className="parts-inventory-row" onClick={()=>openView(x)} role="button" tabIndex="0" onKeyDown={e=>{if(e.key==='Enter'||e.key===' ')openView(x)}}><td><span className="table-primary mono-cell">{val(x.itemCode)||'—'}</span></td><td><span className="table-primary">{val(x.description)||'—'}</span></td><td className="mono-cell">{val(x.controlSerialNo)||'—'}</td><td><span className="parts-branch-badge">{val(x.branch)}</span></td><td className="parts-number">{Number(x.quantity||0).toLocaleString()}</td><td className="parts-number">₱ {money(x.price)}</td><td className="parts-number">₱ {money((Number(x.quantity)||0)*(Number(x.price)||0))}</td></tr>)}</tbody></table></div>
    ...
    {viewing&&<div className="modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)closeView()}}><div className="modal branch-modal parts-view-modal" role="dialog" aria-modal="true" aria-labelledby="parts-view-title"><div className="modal-header"><div><p className="eyebrow">PARTS INVENTORY</p><h2 id="parts-view-title">View Parts</h2><p className="subtext">Inventory item details</p></div><button className="modal-close" onClick={closeView}>×</button></div><div className="modal-body"><div className="parts-view-grid"><div><span>ITEM CODE</span><strong>{val(viewing.itemCode)||'—'}</strong></div><div><span>DESCRIPTION</span><strong>{val(viewing.description)||'—'}</strong></div><div><span>CONTROL / SERIAL NO.</span><strong>{val(viewing.controlSerialNo)||'—'}</strong></div><div><span>BRANCH</span><strong>{val(viewing.branch)||'—'}</strong></div><div><span>QUANTITY</span><strong>{Number(viewing.quantity||0).toLocaleString()}</strong></div><div><span>PRICE</span><strong>₱ {money(viewing.price)}</strong></div><div><span>TOTAL VALUE</span><strong>₱ {money((Number(viewing.quantity)||0)*(Number(viewing.price)||0))}</strong></div></div></div><div className="modal-footer"><button type="button" className="ghost-btn" onClick={closeView}>Close</button><button type="button" className="table-action edit" onClick={()=>{closeView();openEdit(viewing)}}>Update</button><button type="button" className="table-action danger" onClick={()=>{const x=viewing;closeView();remove(x)}}>Delete</button></div></div></div>}

...
const blank={itemCode:'',inventoryId:'',branch:'',assetSerialNo:'',date:'',srf:'',edpStaff:'',status:'NOT DR',notes:''};
const PAGE_SIZE=10;
...
export default function UsedParts(){
  ...
  const markAsDR=async x=>{
    ...
    const ok=window.confirm(`Markahan bilang DR ang Used Parts ${x.itemCode||''} / ${x.assetSerialNo||''}?\n\nAng status nito ay magiging DR.`);
    if(!ok)return;
    try{
      await updateDoc(doc(db,'usedParts',x.id),{status:'DR',updatedAt:serverTimestamp()});
      await audit({action:'MARK_USED_PART_DR',details:`Marked Used Part ${x.itemCode||''} / ${x.assetSerialNo||''} as DR`,targetUserId:x.id});
      await load();
      setViewing(v=>v ? ({...v,status:v.id===x.id?'DR':v.status}) : v);
    }catch(e){setError(e.message||'Unable to update Used Parts status.')}
  };
  ...
  const openEdit=x=>{
    const matchedInventoryId=x.inventoryId || inventory.find(i=>val(i.itemCode).trim().toLowerCase()===val(x.itemCode).trim().toLowerCase() && val(i.controlSerialNo).trim().toLowerCase()===val(x.assetSerialNo).trim().toLowerCase())?.id || '';
    setEditing(x.id);setForm({...blank,...x,inventoryId:matchedInventoryId,date:x.date||''});setError('');setModalOpen(true);document.body.classList.add('modal-open');
  };
  ...
  const selectItemCode=value=>{
    ...
    if(!inv)return setForm(f=>({...f,itemCode:'',inventoryId:'',assetSerialNo:''}));
  ...
  const save=async e=>{
    ...
    try{
      ...
      const normalized=rows.map((r,idx)=>({
        ...r,itemCode:val(r.itemCode).trim(),assetSerialNo:val(r.assetSerialNo).trim(),branch:val(r.branch).trim(),date:val(r.date),srf:val(r.srf).trim(),edpStaff:val(r.edpStaff).trim(),status:r.status==='DR'?'DR':'NOT DR',notes:val(r.notes).trim(),rowIndex:idx
      }));
      for(const r of normalized){
        if(!r.itemCode||!r.inventoryId||!r.branch||!r.date||!r.srf||!r.edpStaff||!r.assetSerialNo)throw new Error(`Complete ang required fields sa Item #${r.rowIndex+1}.`);
      }
      const seen=new Set();
      for(const r of normalized){
        const key=`${r.itemCode.toLowerCase()}|${r.assetSerialNo.toLowerCase()}`;
        if(seen.has(key))throw new Error(`Duplicate ang Item Code at Asset Code sa Item #${r.rowIndex+1}.`);
        seen.add(key);
        const duplicate=items.some(x=>x.id!==editing && val(x.itemCode).trim().toLowerCase()===r.itemCode.toLowerCase() && val(x.assetSerialNo).trim().toLowerCase()===r.assetSerialNo.toLowerCase());
      ...
      await runTransaction(db,async transaction=>{
        ...
        const oldInventoryId=oldRecord?.inventoryId || inventory.find(i=>val(i.itemCode).trim().toLowerCase()===val(oldRecord?.itemCode).trim().toLowerCase() && val(i.controlSerialNo).trim().toLowerCase()===val(oldRecord?.assetSerialNo).trim().toLowerCase())?.id || '';
        ...
        if(editing){
          ...
          const payload={itemCode:r.itemCode,inventoryId:r.inventoryId,branch:r.branch,assetSerialNo:r.assetSerialNo,date:r.date,srf:r.srf,edpStaff:r.edpStaff,status:r.status,notes:r.notes,updatedAt:serverTimestamp()};
          ...
          for(const r of normalized){
            const payload={itemCode:r.itemCode,inventoryId:r.inventoryId,branch:r.branch,assetSerialNo:r.assetSerialNo,date:r.date,srf:r.srf,edpStaff:r.edpStaff,status:r.status,notes:r.notes,updatedAt:serverTimestamp(),createdBy:profile.uid,createdByName:profile.name||profile.username||'',createdAt:serverTimestamp()};
  ...
  const remove=async x=>{
    if(!confirm(`Delete Used Parts record ${x.itemCode||''}? Ibabalik ang 1 quantity sa Parts Inventory.`))return;
    try{
      const inventoryId=x.inventoryId || inventory.find(i=>val(i.itemCode).trim().toLowerCase()===val(x.itemCode).trim().toLowerCase() && val(i.controlSerialNo).trim().toLowerCase()===val(x.assetSerialNo).trim().toLowerCase())?.id || '';
      await runTransaction(db,async transaction=>{
    ...
    }catch(e){setError(e.message||'Unable to delete record.')} 
  };

  const filtered=useMemo(()=>{const q=search.trim().toLowerCase();return items.filter(x=>{const hay=[x.itemCode,x.branch,x.assetSerialNo,x.date,x.srf,x.edpStaff,x.status,x.notes].join(' ').toLowerCase();return (!q||hay.includes(q))&&(branchFilter==='ALL'||x.branch===branchFilter)&&(statusFilter==='ALL'||x.status===statusFilter)})},[items,search,branchFilter,statusFilter]);
  ...
  return <>
    ...
    <div className="content-card table-wrap parts-table used-parts-table"><table><thead><tr><th>ITEM CODE</th><th>BRANCH</th><th>ASSET CODE / SERIAL NO.</th><th>DATE</th><th>SRF</th><th>EDP STAFF</th><th>STATUS</th></tr></thead><tbody>{loading?<tr><td colSpan="7" className="branch-empty">Loading Used Parts...</td></tr>:shown.length===0?<tr><td colSpan="7" className="branch-empty"><div className="branch-empty-icon">◌</div><strong>No Used Parts records found</strong><p>Add a used part record to get started.</p><button className="amber-btn" onClick={openAdd}>＋ Add Used Part</button></td></tr>:shown.map(x=><tr key={x.id} className="clickable-row" onClick={()=>openView(x)} title="Click to view details"><td><span className="table-primary mono-cell">{val(x.itemCode)||'—'}</span></td><td><span className="parts-branch-badge">{val(x.branch)}</span></td><td className="mono-cell">{val(x.assetSerialNo)||'—'}</td><td className="mono-cell">{val(inventory.find(i=>i.id===x.inventoryId)?.controlSerialNo)||'—'}</td><td className="mono-cell">{val(x.srf)||'—'}</td><td><span className="table-primary">{val(x.edpStaff)||'—'}</span></td><td><span className={`used-status ${x.status==='DR'?'dr':'not-dr'}`}>{x.status||'NOT DR'}</span></td></tr>)}</tbody></table></div>
    ...
    {modalOpen&&<div className="modal-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)closeModal()}}><div className="modal branch-modal parts-editor-modal used-parts-modal" role="dialog" aria-modal="true" aria-labelledby="used-parts-editor-modal-title"><div className="modal-header"><div><p className="eyebrow">USED PARTS</p><h2 id="used-parts-editor-modal-title">{editing?'Edit Used Part':'Add Used Parts'}</h2><p className="subtext">{editing?'I-edit ang napiling Used Part record.':'Magdagdag ng maraming Item Code at Asset Code sa iisang Branch. Bawat row ay magbabawas ng 1 sa napiling Parts Inventory item.'}</p></div><button className="modal-close" onClick={closeModal}>×</button></div><form onSubmit={save}><div className="modal-body">
      {editing ? <div className="parts-form-grid">
        ...
        <label>ASSET CODE / SERIAL NO.<input value={form.assetSerialNo} onChange={e=>change('assetSerialNo',e.target.value)} placeholder="Enter Asset Code / Serial No." required/></label>
        ...
        <label>ASSET CODE / SERIAL NO.<input value={row.assetSerialNo} onChange={e=>setBulkRows(rs=>rs.map((r,i)=>i===index?{...r,assetSerialNo:e.target.value}:r))} placeholder="Enter Asset Code / Serial No." required/></label>

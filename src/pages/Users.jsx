...
const blank={name:'',username:'',password:'',role:'employee',employeeId:'',department:'',position:'',status:'active'};
const authEmail=username=>`${String(username).trim().toLowerCase()}@edp-records.local`;
export default function Users(){
 const {profile}=useAuth(); const [users,setUsers]=useState([]); const [form,setForm]=useState(blank); const [editing,setEditing]=useState(null); const [search,setSearch]=useState(''); const [error,setError]=useState(''); const [loading,setLoading]=useState(true);
 const load=async()=>{setLoading(true);const snap=await getDocs(query(collection(db,'users'),orderBy('createdAt','desc')));setUsers(snap.docs.map(d=>({id:d.id,...d.data()})));setLoading(false)};
 useEffect(()=>{load().catch(()=>setLoading(false))},[]);
 const startEdit=u=>{setError('');setEditing(u.id);setForm({...blank,...u,password:'',role:u.role==='super_admin'?'employee':u.role})};
 const cancelEdit=()=>{setEditing(null);setForm(blank);setError('')};
 const submit=async e=>{e.preventDefault();setError('');try{
   if(editing){const target=users.find(u=>u.id===editing);if(profile.role==='super_admin'&&target?.role==='super_admin'){await updateDoc(doc(db,'users',editing),{name:form.name,employeeId:form.employeeId,department:form.department,position:form.position,status:form.status})}else{const nextRole=profile.role==='super_admin'?(form.role==='admin'?'admin':'employee'):form.role;await updateDoc(doc(db,'users',editing),{name:form.name,role:nextRole,employeeId:form.employeeId,department:form.department,position:form.position,status:form.status})}await audit({action:'UPDATE_USER',targetUserId:editing,details:`Updated ${form.username}; role: ${form.role}`})
   }else{
     ...
     await setDoc(doc(db,'users',cred.user.uid),{uid:cred.user.uid,name:form.name,username,role:form.role,employeeId:form.employeeId,department:form.department,position:form.position,status:form.status,createdAt:serverTimestamp()});

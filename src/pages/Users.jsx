...
export default function Users(){
 const {profile}=useAuth(); const [users,setUsers]=useState([]); const [groups,setGroups]=useState([]); const [form,setForm]=useState(blank); const [editing,setEditing]=useState(null); const [search,setSearch]=useState(''); const [error,setError]=useState(''); const [loading,setLoading]=useState(true);
 const load=async()=>{setLoading(true);const [snap,groupSnap]=await Promise.all([getDocs(query(collection(db,'users'),orderBy('createdAt','desc'))),getDocs(query(collection(db,'groups'),orderBy('name','asc')))]);setUsers(snap.docs.map(d=>({id:d.id,...d.data()})));setGroups(groupSnap.docs.map(d=>({id:d.id,...d.data()})));setLoading(false)};
 ...
 const submit=async e=>{e.preventDefault();setError('');try{
   if(editing){const target=users.find(u=>u.id===editing);if(profile.role==='super_admin'&&target?.role==='super_admin'){await updateDoc(doc(db,'users',editing),{name:form.name,employeeId:form.employeeId,department:form.department,position:form.position,groupId:form.groupId||'',groupName:form.groupName||'',status:form.status})}else{const nextRole=profile.role==='super_admin'?(form.role==='admin'?'admin':'employee'):form.role;await updateDoc(doc(db,'users',editing),{name:form.name,role:nextRole,employeeId:form.employeeId,department:form.department,position:form.position,groupId:form.groupId||'',groupName:form.groupName||'',status:form.status})}await audit({action:'UPDATE_USER',targetUserId:editing,details:`Updated ${form.username}; role: ${form.role}`})
   }else{
